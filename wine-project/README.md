...
```
dbx execute --cluster-name=<name of interactive cluster> --job=wine-project-sample-integration-test
```

For a test on an automated job cluster, deploy the job files and then launch:
```
dbx deploy --jobs=wine-project-sample-integration-test --files-only
dbx launch --job=wine-project-sample-integration-test --as-run-submit --trace
```
...
- To trigger the release pipeline, get the current version from the `wine_project/__init__.py` file and tag the current code version:
```
